...
var (
	...
	toTime, fromTime = myt()
	id = "xxx"
	logapi   = "http://xxx/log4x/search/xxx"
	gwapi    = "http://xxx/bap/gw/aopoauth/xxx"
	api      = "http://xxx/bap/gw/rest/xxx?"
...
func Count() string {
  ...
  //统一待办失败数
	...
	//用户登录总数
	sdata = fmt.Sprintf(`{"searchParamHead": {"queryTime": %s}, "searchParamBody": {"requestBody": "{\"indexGroup\": \"log\", \"queryString\": \"sysCode:\\\"unissosrv-app\\\" AND \\\"UaLoginLogServiceImpl.saveLoginLog--request\\\"| stats count(doc_id) as countnum\", \"timeType\":2, \"from\":\"%s\", \"to\":\"%s\"}"}}`, timestr, fromTime, toTime)
	sbody = fmt.Sprintf(`{"indexGroup": "log", "queryString": "sysCode:\"unissosrv-app\" AND \"UaLoginLogServiceImpl.saveLoginLog--request\"| stats count(doc_id) as countnum", "timeType":2, "from":"%s", "to":"%s"}`, fromTime, toTime)
	obj3 := SLogData{
		logdata: sdata,
		logbody: sbody,
		result: "result.result.0.count(doc_id)",
	}

	//用户登录成功数
	sdata = fmt.Sprintf(`{"searchParamHead": {"queryTime": %s}, "searchParamBody": {"requestBody": "{\"indexGroup\": \"log\", \"queryString\": \"sysCode:\\\"unissosrv-app\\\" AND (\\\"UaLoginLogServiceImpl.saveLoginLog--request\\\" AND \\\"成功\\\")| stats count(doc_id) as countnum\", \"timeType\":2, \"from\":\"%s\", \"to\":\"%s\"}"}}`, timestr, fromTime, toTime)
	sbody = fmt.Sprintf(`{"indexGroup": "log", "queryString": "sysCode:\"unissosrv-app\" AND (\"UaLoginLogServiceImpl.saveLoginLog--request\" AND \"成功\")| stats count(doc_id) as countnum", "timeType":2, "from":"%s", "to":"%s"}`, fromTime, toTime)
	obj4 := SLogData{
		logdata: sdata,
		logbody: sbody,
		result: "result.result.0.count(doc_id)",
	...
	//用户登录失败数
	sdata = fmt.Sprintf(`{"searchParamHead": {"queryTime": %s}, "searchParamBody": {"requestBody": "{\"indexGroup\": \"log\", \"queryString\": \"sysCode:\\\"unissosrv-app\\\" AND (\\\"UaLoginLogServiceImpl.saveLoginLog--request\\\" AND (\\\"用户或密码错误\\\" OR \\\"短信验证码\\\"))| stats count(doc_id) as countnum\", \"timeType\":2, \"from\":\"%s\", \"to\":\"%s\"}"}}`, timestr, fromTime, toTime)
	sbody = fmt.Sprintf(`{"indexGroup": "log", "queryString": "sysCode:\"unissosrv-app\" AND (\"UaLoginLogServiceImpl.saveLoginLog--request\" AND (\"用户或密码错误\" OR \"短信验证码\"))| stats count(doc_id) as countnum", "timeType":2, "from":"%s", "to":"%s"}`, fromTime, toTime)
	obj5 := SLogData{
		logdata: sdata,
		logbody: sbody,
		result: "result.result.0.count(doc_id)",
	...
	datasplice = append(datasplice, obj1, obj2, obj3, obj4, obj5)
	totolmap = make(map[int]string)
	for i, data := range datasplice {
		totolmap[i] = zoo(data.logdata, data.logbody, data.result)
	}
	zoodata := fmt.Sprintf(`{"DATE":"%s","LOGINMAX":"%s","LOGINOK":"%s","LOGINERR":"%s","TODOOK":"%s","TODOERR":"%s"}`, date, totolmap[2], totolmap[3], totolmap[4], totolmap[0], totolmap[1])
	return zoodata
}
...
func demo(numberstr, templateid string) bool {
	...
	c := url.Values{"method": {"M_ABILITY_UNINOTI_107000001"}, "format": {"json"}, "appId": {"10009"}, "version": {"V1.0"}, "accessToken": {token}, "sign": {"3D993BD22F44E6C1A638D832DB2BFB6C8BA4116BEE1B3561B4B198DB7BB6B1A0"}, "timestamp": {"20190313162400"},
		"content": {d1}}
	tzapi := fmt.Sprint(api, c.Encode())
	fmt.Println(tzapi)
	code, errstr := GetSendCode(tzapi)
	fmt.Printf("请求统一通知返回的状态码%s,错误信息:%s\n", code, errstr)
	return true
}
